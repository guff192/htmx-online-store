        ...
        if query:
            stmt = (
                select(ProductDbModel._id, ProductDbModel.name, ProductDbModel.description,
                       ...
                       CartProductDbModel.count, CartProductDbModel.selected_configuration_id).
                join(CartProductDbModel,  # joining for getting count from user_product
                     ProductDbModel._id == CartProductDbModel.product_id,
                     isouter=True).
                where(or_(
                ...
                where(  # filtering products in cart and products without user
                    or_(CartProductDbModel.user_id == user_id,
                        CartProductDbModel.user_id == None)).  # noqa: E711
